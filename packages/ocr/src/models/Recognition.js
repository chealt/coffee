...
const decode = (dictionary, textIndex, textProb, isRemoveDuplicate) => {
...
class Recognition extends ModelBase {
  ...
  static async create({ models, onnxOptions = {}, ...restOptions }) {
    const recognitionPath = models?.recognitionPath;
    invariant(recognitionPath, 'recognitionPath is required');
    const dictionaryPath = models?.dictionaryPath;
  ...
  async run(lineImages, { onnxOptions = {} } = {}) {
    const allData = await Promise.all(
      ...
      lineImages.map(async (lineImage, index) => {
        ...
        const modelData = imageToInput(image, {
          // mean: [0.5, 0.5, 0.5],
          // std: [0.5, 0.5, 0.5],
        });
