...
const polygonPolygonArea = (polygon) => {
...
const polygonPolygonLength = (polygon) => {
  const n = polygon.length;
  let i = -1,
    b = polygon[n - 1],
    xa,
...
const orderPointsClockwise = (pts) => {
  const rect = [
    [0, 0],
    [0, 0],
    [0, 0],
...
const linalgNorm = (p0, p1) => Math.sqrt(Math.pow(p0[0] - p1[0], 2) + Math.pow(p0[1] - p1[1], 2));
...
const getRotateCropImage = (imageRaw, points) => {
  ...
  if (dst_img_height / dst_img_width >= 1.5) {
    ...
    const M2 = cv.getRotationMatrix2D(center, 90, 1);
    cv.warpAffine(dst, dst_rot, M2, dsize_rot, cv.INTER_CUBIC, cv.BORDER_REPLICATE, new cv.Scalar());
  }
...
export const splitIntoLineImages = async (image, sourceImage) => {
  const w = image.width;
  ...
  for (let i = 0; i < contours.size(); i++) {
    ...
    const clip = (n, min, max) => Math.max(min, Math.min(n, max));
