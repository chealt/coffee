...
console.log('Copying taste note sub groups into i18n for Polish ...');
await client.batch(
  tasteNoteSubGroups.map(({ id, name }) => ({
...
console.log('Querying taste note groups...');
const { rows: tasteNoteGroups } = await client.execute({
  sql: 'SELECT * FROM taste_note_groups'
});

console.log('Copying taste note groups into i18n for English...');
await client.batch(
...
console.log('Copying taste note groups into i18n for Polish ...');
await client.batch(
  tasteNoteGroups.map(({ id, name }) => ({
...
console.log('Querying taste notes...');
const { rows: tasteNotes } = await client.execute({
  sql: 'SELECT * FROM taste_notes'
});

console.log('Copying taste notes into i18n for English...');
await client.batch(
...
console.log('Copying taste notes into i18n for Polish ...');
await client.batch(
  tasteNotes.map(({ id, name }) => ({
