...
const migrate = async (client) => {
  const result = await client.execute("PRAGMA table_info('collection_item_links')");
  const columnExists = result.rows.some((row) => row.name === 'deleted_at');

  if (!columnExists) {
    console.info('Adding column deleted_at');

    await client.execute('ALTER TABLE collection_item_links ADD COLUMN deleted_at TEXT');
